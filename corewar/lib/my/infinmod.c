...
char *negative_mod(char *nb2)
{
    char *tmp = my_malloc(sizeof(char) * (my_strlen(nb2) + 3));
...
void loop_sub_mod(char *av2, int h, char **tmp_char)
{
    char *verif = negative_mod(av2);
...
{
    int j = 0;
    char *tmp_char = my_malloc(sizeof(char) * (my_strlen(nb1) + 1));
...
{
    char *end = my_malloc(sizeof(char) * (my_strlen(nb2) + 1));
    char *tmp_end = end;
    int boolneg = 0;

    for (int i = 0; i < my_strlen(nb2) + 1; end[i] = '\0', i++);
    boolneg = modneg(&nb1, &nb2);
    end = infinmod(nb1, nb2);
    if (my_strlen(end) == 0) {
        free(tmp_end);
        return my_strdup("0");
    }
    if (boolneg == 1 && my_strcmp(end, my_strdup("error")) != 0)
        end = negative_mod(end);
    return end;
}
